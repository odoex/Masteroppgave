...
% Mesh refinement method with finite volumes for Euler equations with one
% refined grid. 
...
% Mesh refinement: ratio and area of refinement
ratio = 2;
locx = [0.35,0.65];
locy = [0.35,0.65]; 
...
% Speed of sound and max freespeed
c = 331; 
...
if (ratio > 1)
    ...
    location_1 = [(locx(1)-1)*G.h-G.h/(2*ratio),(locy(1)-1)*G.h-G.h/(2*ratio),locx(1),locy(1)];
    
    G_1 = Node(G, location_1, G.h/ratio, k, (locx(2)-locx(1)+1)*ratio, (locy(2)-locy(1)+1)*ratio, n);
    G_1.t = 0;
...
%     G_1.u = initialConditionsEulerConstant(x_1,y_1,0);
...
% Running the scheme: 
tic
G = meshRefinementFunction2(G,t_0,t_n);
toc
