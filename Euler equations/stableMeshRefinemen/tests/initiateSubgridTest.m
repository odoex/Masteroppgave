...
locy = [round(locy(1)/h)+1,round(locy(2)/h)+1]; % [31,70] [0.303,0.697]
...
for k = 1:4
    A(2:end-1,2:end-1,k) = G.child.u(2:r:m_x-1,2:r:m_y-1,k)-G.u(x+1:Mx-1,y+1:My-1,k);
...
isZero = max(max(max(abs(A)))) < 1.0e-12;

assert(max(max(max(abs(A)))) < 1.0e-12)
%assertTrue(isZero)
%assertEquals(max(max(max(abs(A)))),0)
